...
class CableBridgeLKSController extends Controller
{
    ...
    public function gene(Fpdf $fpdf, Request $req){
        ...
        $result = CableBridge::query();
          ...
          $data = $result->select('id','ba', 'bushes_status',
           'vandalism_status', 'pipe_staus','collapsed_status','rust_status','start_date','end_date','visit_date','voltage','coordinate','image_pipe','image_pipe_2','total_defects',
           'image_vandalism','image_vandalism_2','image_collapsed','image_collapsed_2','image_rust','image_rust_2','images_bushes','images_bushes_2')->get();
        // $fpdf->AddPage();
        $fpdf->AddPage('L', 'A4');
        ...
        foreach ($data as $row) {  
            ...
            if ($row->image_vandalism != '' && file_exists(public_path($row->image_vandalism))) {
                ...
                $fpdf->Cell(27, 7, 'no image found', 1); 
            }
        ...
        return $fpdf->output('I','cable_bridge.pdf');
    }
}
